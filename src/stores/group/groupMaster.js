import { ref } from 'vue';
import { defineStore } from 'pinia';
import { makeRequest,state } from "@/requests/requests";



export const useGroupMasterStore = defineStore('groupMaster', () => {
    
    const endpoint='groupMaster'
    const wait=0
    const groupMasterList = ref([]);

    const showCopyJsonModal  = ref(false)
    const copyJsonData = ref({})

    const showViewAllModal = ref(false)
    const viewAllData = ref({})

    const searchInputText = ref('');
    const currentPageNumber = ref(1);
    const totalPages = ref(0);
    const showStartRecords = ref(1);
    const showEndRecords = ref(0);
    const totalRecords = ref(1);
    const currentPageRecords = ref(0); 
    
    async function getGroupMaster(refresh=false) {
        if(refresh){
            state[endpoint]={
              loading: true,
              data: null,
              error: {},
              updating:false
            }
          }
        try {
            var page_id = currentPageNumber.value
            var page_size = 100//selectPageOptions.value[0]
            var search = searchInputText.value

            const response = await makeRequest(endpoint, "GET",{}, {}, {page_id, page_size, search},wait);
            if (response.data){
                groupMasterList.value = response.data.orders? response.data.orders.sort((b, a) => a.id - b.id) : [];

                totalRecords.value = response.data.count || groupMasterList.value.length;
                showStartRecords.value = 1 + (currentPageNumber.value-1) * page_size
                showEndRecords.value = groupMasterList.value.length + (currentPageNumber.value-1) * page_size
                if(totalRecords.value % page_size == 0){
                    totalPages.value = totalRecords.value / page_size 
                } else {
                    totalPages.value = Math.floor(totalRecords.value / page_size) + 1
                }

            }else{
                groupMasterList.value = [];
            }

        } catch (error) {
            console.error('Error fetching group Master:', error);
            throw error;
        }
    }

    getGroupMaster();
    return {
        getGroupMaster,
        groupMasterList,
        showCopyJsonModal,
        copyJsonData,
        showViewAllModal,
        viewAllData,
        endpoint,
        state,

        searchInputText,
        showStartRecords,
        showEndRecords,
        totalRecords,
        currentPageNumber,
        totalPages,
        currentPageRecords,
    };
});